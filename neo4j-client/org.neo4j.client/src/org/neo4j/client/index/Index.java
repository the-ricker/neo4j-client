...
public interface Index<T extends PropertyContainer> extends ReadableIndex<T> {
	 ...
	 * pair for the entity is already in the index it's up to the implementation
	 * to make it so that such an operation is idempotent.
	 * 
	 * @param entity
	 *            the entity (i.e {@link Node} or {@link Relationship}) to
	 *            associate the key/value pair with.
	 * @param key
	 ...
	 * @param entity
	 *            the entity (i.e {@link Node} or {@link Relationship}) to
	 *            dissociate the key/value pair from.
	 * @param key
	 *            the key in the key/value pair to dissociate from the entity.
	 * @param value
	 *            the value in the key/value pair to dissociate from the entity.
	 */
	void remove(T entity, String key, Object value);

	/**
	 * Removes key/value pairs for {@code entity} where key is {@code key} from
	 * the index.
	 * 
	 * Implementations can choose to not implement this method and should in
	 * that case throw {@link UnsupportedOperationException}.
	 * 
	 * @param entity
	 *            the entity ({@link Node} or {@link Relationship}) to remove
	 *            the this index.
	 */
	void remove(T entity, String key);

	/**
	 * Removes an entity from the index and all its key/value pairs which has
	 * been previously associated using
	 ...
	 * transaction which the delete operation was called in gets rolled back it
	 * again becomes ok to use this index.
	 */
	void delete();

	/**
	 * Add the entity to this index for the given key/value pair if this
	 * particular key/value pair doesn't already exist.
	 * 
	 * This ensures that only one entity will be associated with the key/value
	 * pair even if multiple transactions are trying to add it at the same time.
	 * One of those transactions will win and add it while the others will
	 * block, waiting for the winning transaction to finish. If the winning
	 * transaction was successful these other transactions will return the
	 * associated entity instead of adding it. If it wasn't successful the
	 * waiting transactions will begin a new race to add it.
	 * 
	 * @param entity
	 *            the entity (i.e {@link Node} or {@link Relationship}) to
	 *            associate the key/value pair with.
